...
`memento` is a strict single-user personal **poster wall (海报墙)** with browser login, private diaries and a small admin settings page. The sole browser account is the administrator; multiple sessions mean multiple devices, not users or RBAC. Collection reads are public and collection writes use `X-API-Key`; diary API reads and writes both require that key. Browser diary/admin access uses a separate cookie session. Release frontend assets (`static/`) are embedded via `rust-embed`; business data remains in external SQLite and non-secret feature settings in external YAML.
...
No-argument server startup selects `MEMENTO_ENV=development|production` from the system environment first, defaulting to production. Load only cwd `.env.development` or `.env.production`; never load generic `.env`, search parents, override existing system values or let the file switch mode. `hash-password`, `totp-secret` and `init-db` dispatch before dotenv/server startup and load no env or YAML. Development defaults: bind `0.0.0.0:23457`, DB `./memento.dev.db`, YAML `./memento.development.yaml`; production defaults: bind `127.0.0.1:23457`, DB `./memento.db`, YAML `./memento.production.yaml`, all relative to cwd. Launch production with `MEMENTO_ENV=production ./memento`. Unset/blank API Key disables all key routes without logging a secret. Never commit actual environment files, runtime YAML or credentials; example hash/secret values stay empty and `memento.example.yaml` contains no secrets.

Keep configuration ownership explicit: Env is only for runtime/security/secrets/business rules, YAML only for admin-editable non-secret feature settings, and SQLite for business data. SQLite schema remains v3 and the diary importer is unchanged. The server creates a missing selected YAML only when its parent already exists (0600 on Unix); legacy `MEMENTO_SITE_NAME` / `MEMENTO_SLOGAN` / `MEMENTO_ICON` seed that first file once and are ignored/deprecated afterward. Existing invalid YAML must reject startup without overwrite. Programmatic saves reserialize the file and drop comments. This is single-instance, with no external-file watch or merge. See [admin settings design](docs/admin-settings-design.md).
...
- **`settings.rs`** owns strict versioned YAML validation and same-directory temporary-file persistence. File `rename` is the commit point; only then update the in-memory snapshot, followed by best-effort parent sync. Do not add multi-instance coordination or file watching implicitly. `handlers/settings.rs` exposes only session-protected `GET/PUT /private/settings/site`; PUT also requires CSRF and production Origin, and API Key cannot substitute.

## Private diaries and browser login

Authoritative details: [design](docs/diary-design.md), [API](docs/diary-api.md), [operations and migration](docs/diary-operations.md), and [admin/YAML design](docs/admin-settings-design.md). Operations records historical local validation separately from unverified changes, deployment, real-data migration and cross-platform behavior.
...
- `/private/settings/site` follows the same browser session boundary: GET reads public site fields; PUT is a full replacement requiring CSRF and production Origin, with a 512 KiB JSON limit. Site name is 1..80 Unicode scalars, slogan 0..200, and icon is the built-in default, a constrained HTTPS URL, or a base64 raster data URI with matching magic bytes and at most 256 KiB decoded. Preserve strict unknown-field rejection and HTML context escaping.
...
- Shared navigation covers collection, diary, login and admin. Account discovery uses same-origin `/session`; login `next` permits only `/diary` or `/admin` and defaults to admin. The collection page no longer has visible name search, but public API `q` remains compatible.
