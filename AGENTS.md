...
- Rust 单二进制个人收藏海报墙与私有连续日记。电影／游戏／图书收藏公开浏览；日记仅本人登录或持有 API Key 的程序可读写。第一期不含目标、看板、多用户、审计或回收站。
- release 内嵌静态前端，收藏、海报、日记和会话持久化在外部 SQLite，**数据不打包进二进制**。`static/` 保持原生 HTML/CSS/JS，无 npm 或打包器。
...
| `src/config.rs`、`src/browser.rs` | 站点/环境配置；浏览器认证配置、密码验证、持久会话及 CSRF |
...
| `src/assets.rs`、`static/` | 静态资源、首页文本注入；日记页 `/diary` 仅是无私密数据的外壳 |
...
- `/api/favorites` 公开读、API Key 写；`/api/diaries` **所有读写均需 `X-API-Key`**；`/private/diaries` 是同源浏览器会话入口。两组日记路由复用同一业务 handler，Cookie 与 API Key 不互相替代，也不因浏览器登录开放收藏写入。
...
- 保留 `nosniff`、`DENY`、`no-referrer`；日记/会话响应 no-store。API 文本只用 textContent，来源链接限 HTTP(S)，首页 SITE_NAME/SLOGAN/ICON 保持对应 HTML 文本/属性转义。
